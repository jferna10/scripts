...
public class strict_dreme_neg
{
	public strict_dreme_neg(String pos, String neg)
	{
		try
		{
			...
			FileWriter dneg = new FileWriter(new File("temp.tmp"));		//write the chopped sequences
			...
			while (nline != null)
			{
				...
				while ((pline != null) && !found)
				{
					...
					pt_start = Integer.valueOf(tk.nextToken());//target start in consensus
					pt_end = Integer.valueOf(tk.nextToken());
		...
		try
		{
			...
			String outfile = "strict_map_"+ noext + ".fa";
			String infile = noext + ".fa";
			
			
			BufferedReader tmp = new BufferedReader(new FileReader("temp.tmp"));
			...
			String line = tmp.readLine(); //line contains the just written list of negative names
			

			while (line != null)
			{
				BufferedReader in = new BufferedReader(new FileReader(infile)); 
				String fa = in.readLine();
				
				boolean found = false;
				while ((fa != null) && !found)
				{
					StringTokenizer tok = new StringTokenizer(fa, ">");
					//tok.nextToken();
